...
game.showChallenge = function() {
  if (challenge.isActive && challenge.difficulty == game.difficulty) {
    $("#challengeArea").text("Incoming challenge! Beat "
...
game.checkChallenge = function() {
  if (challenge.isActive && challenge.difficulty == game.difficulty) {
    if (game.finalScore > challenge.scoreToBeat) {
      ...
      $('#challengeArea').text('Challenge not beaten. Try again!');
    }
...
game.requestScore = function() {
  if (game.gameOver) {
    ...
    if (game.difficulty == game.EASY) {
      ...
      $("#gameMessageArea").text("Well, okay then. Your final score is "
        + game.finalScore);

    } else {
      game.finalScore = Math.ceil(requestedScore / 2.0);
      $("#gameMessageArea").text(
        "What, you thought it would be that easy? Your final score is "
        + game.finalScore);
    }
...
game.showLeaderboard = function() {
  ...
  leaderboardTable.showLeaderboard(leaderboardToShow,
    leaderboardTable.BACK_TO_GAME);
  $('#game').hide();
