...
graphics.drawAlly = function(ctx, a) {
  ...
  if (graphics.ovum) {
    ...
    for (var i = 0; i < 5; i++) {
      graphics.drawBox(ctx, a.x + ALLY_W, a.y + i * ALLY_H / 5,
        SCREEN_W, ALLY_H / 5, c[i]);
    }
  }
};
...
// Draw the bullet
graphics.drawBullet = function(ctx, x, y, h) {
  graphics.drawBox(ctx, x, y, BULLET_W, h, graphics.palette.player);  
};
