...
gamelogic.doFrame = function() {
  ...
  if (gamelogic.game.nextSpawn < now)
  ...
  var palNo = (combo >= PALETTE_FOR_COMBO.length)
    ? 0 : PALETTE_FOR_COMBO[combo];
  ...
  if (continueGame) {
    if (window.requestAnimationFrame) {
      ...
      setTimeout(gamelogic.doFrame, 1);
  }
};
...
gamelogic.update = function(delta) {
  ...
  for (var c = 0; c <= 1; c++) {
    ...
    for (var i = coll.length - 1; i >= 0; --i) {
      ...
      if (coll[i].x < -coll[i].w)
        util.removeAt(coll, i);
    }
  }
  ...
  if (gamelogic.game.bullet) {
    ...
    var bulletSpeed = BULLET_SPEED + gamelogic.game.kills * ENEMY_SPEEDUP_UNIT;
    ...
    if (gamelogic.game.bullet.y < -BULLET_H) {
      ...
      gamelogic.game.bullet.h = SCREEN_H - gamelogic.game.bullet.y;
      gamelogic.checkBulletHit();
...
gamelogic.checkBulletHit = function() {
  ...
  for (var c = 0; c <= 1; c++) {
    ...
    for (var i = collection.length - 1; i >= 0; --i) {
      ...
      if (gamelogic.game.score < 0)
        gamelogic.game.score = 0;
...
gamelogic.checkWallHitWith = function(e) {
  if (e.x >= gamelogic.game.wallLeft)
    return false;
...
gamelogic.checkPrecisionAchievements = function() {
  for (var i = 0; i < constants.ACHIEVEMENTS.PRECISION.length; ++i) {
    var a = constants.ACHIEVEMENTS.PRECISION[i];
...
gamelogic.checkIntegrityAchievements = function() {
  for (var i = 0; i < constants.ACHIEVEMENTS.INTEGRITY.length; ++i) {
    var a = constants.ACHIEVEMENTS.INTEGRITY[i];
...
gamelogic.checkRankAchievements = function() {
  for (var i = 0; i < constants.ACHIEVEMENTS.RANK.length; ++i) {
    var a = constants.ACHIEVEMENTS.RANK[i];

    if (gamelogic.game.score >= a.score)
      gameservices.unlockAchievement(a.id);
  }
};
// =============================================================================
...
gamelogic.checkIncrementalAchievements = function(endOfGame) {
  ...
  if (tooSoon && !endOfGame)
    return; // throttle

  if (gamelogic.game.killsToSend > 0) {
    for (var i = 0; i < constants.ACHIEVEMENTS.EXPERIENCE.length; ++i) {
      var a = constants.ACHIEVEMENTS.EXPERIENCE[i];
  ...
  if (endOfGame) {
    ...
    gameservices.incrementAchievement(constants.ACHIEVEMENTS.FREQUENT, 1);

    if (gamelogic.game.score >= constants.ACHIEVEMENTS.SERIOUS.minScore) {
      gameservices.incrementAchievement(
        constants.ACHIEVEMENTS.SERIOUS.id, 1);
    }
  }
};
...
gamelogic.ovumProc = function(code) {
  ...
  if (o.p == 657273 && o.q == 370000 && o.r == 699040 && o.s == 5) {
    ...
    gameservices.unlockAchievement(constants.ACHIEVEMENTS.OVUM);

    if (gamelogic.game.score % 10 != 1)
      gamelogic.game.score += 1;
  }
};
