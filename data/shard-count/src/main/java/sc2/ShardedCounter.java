...
public final class ShardedCounter {
	...
	/**
	 * Entity kind prefix, which is concatenated with the counter name to form the
	 * final entity kind, which represents counter shards.
	 */
	 ...
	 */
	private static final DatastoreService DS
	 ...
	 */
	public ShardedCounter(String name) {
		counterName = name;
	...
	public void addShards(int count) {
		Key counterKey = KeyFactory.createKey(KIND, counterName);
		incProp(counterKey, SHARD_COUNT, count, INITIAL_SHARDS + count);
	}
	 ...
	 * the provided key does not exist then create an entity with the supplied
	 ...
	 */
	private void incProp(Key key, String prop, long increment, 
					long initialValue) {
		...
		try {
			...
			value = (Long) entity.getProperty(prop) + increment;
